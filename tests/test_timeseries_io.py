"""Tests for :mod:`synarius_core.io.timeseries` (no GUI)."""
...
from synarius_core.io.timeseries import TimeSeriesBundle, load_timeseries_file
...
def test_export_recording_buffers_parquet(tmp_path: Path) -> None:
    out = tmp_path / "rec.parquet"
    series = {
        "v_a": ([0.0, 1.0, 2.0], [1.0, 1.1, 1.2]),
        "v_b": ([0.0, 1.0, 2.0], [2.0, 2.1, 2.2]),
    }
    export_recording_buffers(series, out, fmt="parquet")
    assert out.is_file()
    b = load_timeseries_file(out)
    assert b.format == "parquet"
    assert set(b.data.columns) == {"v_a", "v_b"}
    t, ya = b.get_series("v_a")
    _, yb = b.get_series("v_b")
    assert len(t) == 3
    assert np.allclose(ya, np.array([1.0, 1.1, 1.2]))
    assert np.allclose(yb, np.array([2.0, 2.1, 2.2]))
