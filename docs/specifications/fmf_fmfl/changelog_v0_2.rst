...
* **Synarius Core pipeline** — :doc:`architecture_and_pipeline_v0_2`, §J.1–§J.2, describes the **scalar dataflow** implementation: **delayed feedback** (unit delay on inferred feedback edges), **FMU-on-cycle** rejection, optional **scalar slot fusion** via ``dataflow.scalar_slot_id``.
